...
const SelectorWithSearchAndPagination = ({
  searchVal = "",
  selected = null,
  onSelect,
  className = "",
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        setLoading(true);
        const resData = await getDetails({
          search,
          page: page,
          limit: 7,
        });
      ...
      {isOpen && (
        <div className="absolute left-0 right-0 mt-1 bg-white border rounded shadow-lg z-50 animate-fade">
          ...
          <div className="flex gap-x-2 relative">
            ...
            {selected && (
              <button
                className={`absolute p-1 w-fit outline-none bg-red-400 text-white cursor-pointer hover:bg-red-500 right-0 top-1/2 -translate-y-1/2 hover:text-gray-100 rounded-full ${cancelClassName}`}
                onClick={handleClear}
              >
                <RxCross2 />
              </button>
          ...
          <div className="max-h-56 overflow-y-auto">
            {loading && !data.length === 0 ? (
              ...
              data.map((item) => (
                ...
                >
                  <span className="mr-2.5">{item[valKey]}</span>
                </div>
              ))
            ) : (
...
export default SelectorWithSearchAndPagination;
