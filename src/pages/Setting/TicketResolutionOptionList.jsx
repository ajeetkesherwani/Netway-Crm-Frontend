...
export default function TicketResolutionOptionList() {
  ...
  const fetchOptions = async () => {
    ...
    try {
      ...
      else setError(res.message || "Failed to load resolution options");
    } catch (err) {
  ...
  const confirmDelete = async () => {
    ...
    try {
      const res = await deleteTicketResolutionOption(id);
      if (res && (res.status === true || res.success)) {
        await fetchOptions();
      } else if (res && res.message) {
        setError(res.message);
      } else {
        setError("Failed to delete resolution option");
      }
    } catch (err) {
      setError(err?.message || "Network error");
    } finally {
  ...
  if (loading) return <p className="p-4">Loading resolution options...</p>;
  ...
  return (
    <div className="p-6">
      ...
      {/* Empty State */}
      {options.length === 0 ? (
        <p className="text-gray-500">No ticket resolution options found.</p>
      ) : (
        <>
          {/* ✅ Desktop Table View */}
              ...
              <tbody className="divide-y divide-gray-200">
                {options.map((opt, index) => (
                  <tr key={opt._id} className="hover:bg-gray-50">
                    ...
                    <td className="px-[2px] py-[6px]">{opt.name}</td>
              ...
              >
                ...
                <div className="flex justify-end space-x-3 mt-3">
                  <button
                    onClick={() =>
                      navigate(
                        `/setting/ticketResolutionOption/update/${opt._id}`
                      )
                    }
