...
// Stage removed, adding categories if needed, but for now just New Connection Request
...
const stageBadge = (stage) => {
  ...
  return `px-2 py-0.5 rounded text-xs font-semibold ${map[stage] || "bg-gray-100 text-gray-600"}`;
};
...
const severityBadge = (sev) => {
  ...
  return `px-2 py-0.5 rounded text-xs font-semibold ${map[sev] || "bg-gray-100 text-gray-600"}`;
};
...
export default function ManageLead() {
  ...
  }, [name, mobile, category, fromDate, toDate]);
  ...
  return (
    <div className="p-4 md:p-6 bg-gray-50 min-h-screen flex flex-col">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-5 gap-3">
        ...
        <ProtectedAction module="lead" action="Create">
          <button
            onClick={() => navigate("/sales/lead/create")}
      ...
      <div className="bg-white border rounded-xl shadow-sm p-4 mb-5">
        ...
        {(name || mobile || category || fromDate || toDate) && (
          <button
            onClick={() => {
              setName(""); setMobile(""); setCategory(""); setFromDate(""); setToDate("");
            }}
              ...
              <tbody>
                {leads.map((lead, idx) => (
                  <tr key={lead._id} className="border-b hover:bg-gray-50 transition">
                    ...
                    <td className="py-2.5 px-4 font-mono text-xs text-blue-700 font-semibold">
                      {lead.leadNumber}
                    </td>
                    ...
                    <td className="py-2.5 px-4">
                      <span className="px-2 py-1 bg-gray-100 text-gray-700 text-xs rounded font-semibold">{lead.category || "-"}</span>
                    </td>
                    <td className="py-2.5 px-4">
