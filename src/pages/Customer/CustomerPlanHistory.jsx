...
import { useParams, useNavigate } from "react-router-dom";
import { useEffect, useState } from "react";
import { getUserPlanHistory, getUserRenewHistory, getUserRefundHistory } from "../../service/user";
import { toast } from "react-toastify";
import { FaArrowLeft } from "react-icons/fa";
  ...
  useEffect(() => {
    const loadData = async () => {
      ...
      try {
        ...
        // Fetch refunds
        const refundResult = await getUserRefundHistory(id);
        if (refundResult.status) {
          const sortedRefunds = refundResult.data.sort(
      ...
      <div className="max-w-8xl mx-auto">
        ...
        {!loading && !error && (
          <div className="space-y-10">
            ...
            {/* ──────────────────────────────────────── */}
            <section>
              <h1 className="text-3xl font-bold text-gray-800 mb-6">Refund History</h1>

              {refunds.length === 0 ? (
                <div className="text-center py-12 bg-white rounded-lg shadow">
                  <p className="text-gray-500 text-lg">No refund history found</p>
                </div>
              ) : (
                <div className="bg-white rounded-lg shadow overflow-hidden">
                  <div className="overflow-x-auto">
