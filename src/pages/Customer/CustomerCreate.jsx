...
export default function CreateUser() {
  ...
  useEffect(() => {
    const fetchAll = async () => {
      try {
        ...
        if (rRes.status === "fulfilled" && rRes.value?.status && rRes.value.data)
        ...
        if (lcoRes.status === "fulfilled" && lcoRes.value?.status && lcoRes.value.data)
          setLcos(lcoRes.value.data);

        if (staffRes && staffRes.status === "fulfilled" && staffRes.value?.status && staffRes.value.data)
    ...
    if (type === "checkbox") setFieldValue(path, checked);
      ...
      if (fileList && fileList.length) {
        ...
        if (path === "documents") {
          ...
          setFormData((prev) => ({ ...prev, documents: [...prev.documents, ...arr] }));
        } else {
  ...
  const handleInstallationByToggleManual = () => {
    ...
    setFormData((prev) => ({
      ...
      customer: {
        ...prev.customer,
        installationByManual: prev.customer.installationByManual ? "" : prev.customer.installationByManual,
      },
    }));
  };
  ...
  const validateForm = () => {
    ...
    else if (!/^\S+@\S+\.\S+$/.test(email)) errors["customer.email"] = "Invalid email";

    // mobile
    const mobile = (c.mobile || "").trim();
    if (!mobile) errors["customer.mobile"] = "Mobile is required";
    else if (!/^\d{10,}$/.test(mobile)) errors["customer.mobile"] = "Enter valid mobile (10+ digits)";

    // alternate mobile optional but if filled validate
    const alt = (c.alternateMobile || "").trim();
    if (alt && !/^\d{10,}$/.test(alt)) errors["customer.alternateMobile"] = "Enter valid mobile";

    // aadhar basic check (12 digits)
    const aadhar = (c.aadharNo || "").trim();
    if (aadhar && !/^\d{12}$/.test(aadhar)) errors["customer.aadharNo"] = "Aadhar must be 12 digits";
    ...
    if (!bill.state) errors["addresses.billing.state"] = "Billing state required";
    if (!bill.city) errors["addresses.billing.city"] = "Billing city required";
    if (!bill.pincode) errors["addresses.billing.pincode"] = "Billing pincode required";
    ...
    <div className="max-w-[1400px] mx-auto p-4 bg-white shadow rounded">
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <section className="border rounded">
          <div className="bg-blue-800 text-white px-4 py-2 font-semibold">Customer Details</div>
          <div className="p-4 grid grid-cols-1 md:grid-cols-4 gap-4">
            ...
            <div>
              ...
              <input
                ...
                placeholder="Name"
              />
              {formErrors["customer.name"] && <p className="text-red-500 text-sm">{formErrors["customer.name"]}</p>}
            </div>
            ...
            <div>
              ...
              <label className="inline-flex items-center mt-1 text-sm">
                <input
                  ...
                  onChange={(e) => handleChange(e, "customer.differentBillingName")}
                  className="mr-2"
                />
                Different From Name
              </label>
            ...
            <div>
              ...
              <input
                ...
                placeholder="Email"
              />
              {formErrors["customer.email"] && <p className="text-red-500 text-sm">{formErrors["customer.email"]}</p>}
            </div>
            ...
            <div>
              ...
              />
              {formErrors["customer.mobile"] && <p className="text-red-500 text-sm">{formErrors["customer.mobile"]}</p>}
            </div>
            ...
            <div>
              ...
              />
              {formErrors["customer.alternateMobile"] && <p className="text-red-500 text-sm">{formErrors["customer.alternateMobile"]}</p>}
            </div>

            <div>
              <label className="block text-sm font-medium">Account Id (IPACCT Id)</label>
              <input
            ...
            <div className="md:col-span-2">
              <label className="block text-sm font-medium">Installation By</label>
              {/* multi select */}
              <select
                value={formData.customer.salesExecutiveId}
              ...
              <div className="mt-2">
                <label className="block text-sm font-medium">Or Enter Manual Installer</label>
                <input
                  value={formData.customer.installationByManual}
                  onChange={(e) => handleChange(e, "customer.installationByManual")}
                  className="mt-1 p-2 border rounded w-full"
            ...
            {formData.customer.ipType === "Dynamic IP Pool" && (
              <div>
                <label className="block text-sm font-medium">Dynamic IP Pool</label>
                <input
        ...
        {/* ---------------- Address Details ---------------- */}
        <section className="border rounded">
          <div className="bg-blue-800 text-white px-4 py-2 font-semibold">Address Details</div>
          <div className="p-4 grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div className="border rounded p-3">
              ...
              <input
                ...
                onChange={(e) => handleChange(e, "addresses.billing.addressLine1")}
              ...
              <input
                ...
                onChange={(e) => handleChange(e, "addresses.billing.addressLine2")}
                className="mt-1 p-2 border rounded w-full"
              />
              <div className="flex gap-2 mt-2">
                ...
                <input
                  ...
                  placeholder="State *"
                  className={`p-2 border rounded w-1/2 ${formErrors["addresses.billing.state"] ? "border-red-500" : ""}`}
                />
              </div>
              <div className="flex gap-2 mt-2">
                <input
            ...
            <div className="border rounded p-3">
              ...
              <label className="inline-flex items-center text-sm">
                <input
                  ...
                  onChange={(e) => handleChange(e, "addresses.installation.sameAsBilling")}
                  className="mr-2"
                />
              ...
              {!formData.addresses.installation.sameAsBilling && (
                <>
                  <input
                    ...
                    onChange={(e) => handleChange(e, "addresses.installation.addressLine1")}
                    className="mt-2 p-2 border rounded w-full"
                    placeholder="Address Line 1"
                  />
                  <input
                    value={formData.addresses.installation.addressLine2}
                    onChange={(e) => handleChange(e, "addresses.installation.addressLine2")}
                    className="mt-2 p-2 border rounded w-full"
                  ...
                  <div className="flex gap-2 mt-2">
                    <input
                      ...
                      onChange={(e) => handleChange(e, "addresses.installation.city")}
                      placeholder="City"
                      className="p-2 border rounded w-1/2"
                    />
                    <input
                      value={formData.addresses.installation.state}
                      onChange={(e) => handleChange(e, "addresses.installation.state")}
                      placeholder="State"
                      className="p-2 border rounded w-1/2"
                    />
                  </div>
                  <div className="flex gap-2 mt-2">
                    <input
                      ...
                      onChange={(e) => handleChange(e, "addresses.installation.pincode")}
                      placeholder="Pincode"
                      className="p-2 border rounded w-1/2"
                    />
                  </div>
            ...
            <div className="md:col-span-3 mt-2 border rounded p-3">
              ...
              <div className="space-y-2 mt-2">
                ...
                {/* <button type="button" onClick={addArea} className="px-3 py-1 bg-green-600 text-white rounded">
                  + Add Area
                </button> */}
              </div>
            </div>
        ...
        <section className="border rounded">
          <div className="bg-blue-800 text-white px-4 py-2 font-semibold">Network & Package</div>
          <div className="p-4 grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              ...
              <input
                ...
                onChange={(e) => setFieldValue("customer.packageDetails.packageName", e.target.value)}
                className="mt-1 p-2 border rounded w-full"
              />
              <input
                placeholder="Amount"
                value={formData.customer.packageDetails.packageAmount}
                onChange={(e) => setFieldValue("customer.packageDetails.packageAmount", e.target.value)}
                className="mt-1 p-2 border rounded w-full"
              />
            </div>
            <div>
        ...
        <section className="border rounded">
          ...
          <div className="p-4">
            ...
            {formData.documents.map((doc, index) => (
              <div key={index} className="grid grid-cols-1 md:grid-cols-3 gap-3 mb-4 p-3 border rounded">
                  ...
                  >
                    ...
                    {documentTypes.map((dt) => (
                      <option
                        key={dt}
                        value={dt}
                        disabled={formData.documents.some((d, i) => d.type === dt && i !== index)} // prevent duplicates
                      >
                        {dt}
                      </option>
                    ))}
                ...
                <div className="md:col-span-1">
                  ...
                  <input
                    type="file"
                    onChange={(e) => updateDocumentFile(index, e.target.files[0])}
                    className="mt-1 p-2 border rounded w-full"
                    disabled={!doc.type}
                  />
                  {doc.file && <p className="text-sm mt-1 text-gray-700">{doc.file.name}</p>}
                </div>
            ...
            {formData.documents.length === 0 && (
              <p className="text-sm mt-2 text-gray-500">No documents added yet</p>
            )}
          </div>
        </section>
        ...
        {/* ---------------- Additional & Actions ---------------- */}
        <section className="border rounded">
          <div className="bg-blue-800 text-white px-4 py-2 font-semibold">Additional</div>
          <div className="p-4 grid grid-cols-1 md:grid-cols-3 gap-4">
            <div>
            ...
            <div>
              ...
              <select
                ...
                onChange={(e) => setFieldValue("additional.status", e.target.value === "Active")}
                className="mt-1 p-2 border rounded w-full"
              >
          ...
          <div className="p-4 flex justify-end gap-3">
            <button type="button" onClick={() => navigate("/user/list")} className="px-4 py-2 bg-gray-500 text-white rounded">
              Back
            </button>
            ...
            <button type="button" onClick={handleClear} className="px-4 py-2 bg-red-600 text-white rounded">
              Clear
            </button>
          </div>
        </section>
