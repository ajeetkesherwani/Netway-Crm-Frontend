...
const AsyncResellerSelect = ({ onSelect, className = "" }) => {
  ...
  return (
    <div className={`relative ${className}`} ref={dropdownRef}>
      ...
      >
        {selected ? selected.resellerName : "Select Reseller..."}
      </div>
              ...
              data.map((item) => (
                ...
                >
                  <span className="mr-2.5">
                    {item.resellerName || item.mobileNo || item.email}
                  </span>
                </div>
              ))
            ) : (
...
export default AsyncResellerSelect;
