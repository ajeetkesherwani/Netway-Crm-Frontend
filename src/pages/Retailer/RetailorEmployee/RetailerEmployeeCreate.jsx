...
import { addEmployee } from "../../../service/retailerEmployee";


export default function RetailerEmployeeCreate() {
  const navigate = useNavigate();
  const {  lcoId } = useParams();
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true);
    try {
      await addEmployee(lcoId, formData);
      toast.success("Employee added successfully ✅");
      // navigate(`/retailer/employee/list/${lcoId}`);
      navigate(`/retailer/list/${lcoId}`);
    } catch (err) {
    ...
    <div className="max-w-7xl mx-auto p-6 bg-white shadow rounded">
      ...
      <form onSubmit={handleSubmit} className="grid grid-cols-2 gap-4">
        ...
        <div className="col-span-2 flex justify-end gap-3 mt-4">
          <button
            type="button"
            onClick={() => navigate(`/retailer/employee/list/${lcoId}`)}
