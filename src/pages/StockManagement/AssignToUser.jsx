...
import { assignStockToUser, getStockCategories } from "../../service/stockCategory";
...
export default function AssignToUser() {
  ...
  const fetchUsers = async () => {
    try {
      ...
      const userList = Array.isArray(data) ? data : (data?.data?.users || data?.users || data?.data || []);
  ...
  const handleAssign = async (e) => {
    e.preventDefault();
    if (!selectedStocks.length || !selectedUser) {
      toast.error("Please select at least one stock item and a user");
      return;
    }
    ...
    try {
      const promises = selectedStocks.map(stockOpt => 
        assignStockToUser(stockOpt.value, { userId: selectedUser.value, comment })
      );
      
      await Promise.all(promises);
      toast.success("Stock assigned to user successfully");
      setSelectedStocks([]);
  ...
  const userOptions = users.map(u => ({
    value: u._id || u.id,
    label: u.generalInformation?.name || u.name || "User"
  }));
    ...
    <div className="p-6 bg-gray-50 min-h-full">
      <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6 w-full mx-auto">
        <div className="flex justify-between items-start mb-6">
          <div>
            <h2 className="text-xl font-bold text-gray-800 mb-1">Assign Stock to User</h2>
            <p className="text-sm text-gray-500">Select multiple available stock items and assign them to a customer/user.</p>
          </div>
          <button
            onClick={() => navigate(-1)}
