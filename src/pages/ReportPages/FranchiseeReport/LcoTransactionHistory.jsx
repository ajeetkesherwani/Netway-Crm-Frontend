...
import { getLcoTransactionHistoryReport } from "../../../service/franchiseeReports";
import { IoMdArrowBack } from "react-icons/io";

export default function LcoTransactionHistory() {
  ...
  // Fetch LCO transaction history report
  useEffect(() => {
    const loadTransactions = async () => {
      setLoading(true);
      try {
        const res = await getLcoTransactionHistoryReport(page, limit, searchTerm);
        setTransactions(res.data.data || []);
        setTotalPages(Math.ceil(res.data.total / limit));
      } catch (err) {
        console.error("Error fetching LCO transaction history report:", err);
        setError("Failed to load LCO transaction history report");
        toast.error("Failed to load LCO transaction history report ❌");
      } finally {
        setLoading(false);
      }
    };
  ...
  return (
    <div className="p-6">
      <div className="flex items-center justify-between mb-4">
        <h1 className="text-xl font-semibold">LCO Transaction History</h1>
      </div>
      <button
