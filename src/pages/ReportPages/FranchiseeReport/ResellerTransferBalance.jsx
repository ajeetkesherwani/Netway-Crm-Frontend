...
import { getResellerTransferBalanceReport } from "../../../service/franchiseeReports";
import { IoMdArrowBack } from "react-icons/io";

export default function ResellerTransferBalance() {
    ...
    // Fetch reseller transfer balance report
    useEffect(() => {
        const loadTransactions = async () => {
            setLoading(true);
            try {
                const res = await getResellerTransferBalanceReport(page, limit, searchTerm);
                setTransactions(res.data.data || []);
                setTotalPages(Math.ceil(res.data.total / limit));
            } catch (err) {
                console.error("Error fetching reseller transfer balance report:", err);
                setError("Failed to load reseller transfer balance report");
                toast.error("Failed to load reseller transfer balance report ❌");
            } finally {
                setLoading(false);
            }
        };
    ...
    return (
        <div className="p-6">
            <div className="flex items-center justify-between mb-4">
                <h1 className="text-xl font-semibold">Reseller Transfer Balance</h1>
            </div>
            <button
