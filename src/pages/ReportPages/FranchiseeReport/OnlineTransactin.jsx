...
import { getOnlineTransactionReport } from "../../../service/franchiseeReports";
...
export default function OnlineTransaction() {
    ...
    // Fetch online transaction report
    useEffect(() => {
        const loadTransactions = async () => {
            setLoading(true);
            try {
                const res = await getOnlineTransactionReport(page, limit, searchTerm);
                setTransactions(res.data.data || []);
                setTotalPages(Math.ceil(res.data.total / limit));
            } catch (err) {
                console.error("Error fetching online transaction report:", err);
                setError("Failed to load online transaction report");
                toast.error("Failed to load online transaction report ❌");
            } finally {
                setLoading(false);
            }
        };
    ...
    return (
        <div className="p-6">
            <div className="flex items-center justify-between mb-4">
                <h1 className="text-xl font-semibold">Online Transaction</h1>
            </div>
            <button
                            ...
                            <tbody className="divide-y divide-gray-200">
                                {transactions.map((transaction, index) => (
                                    <tr key={transaction._id} className="hover:bg-gray-50 relative">
                                        ...
                                        <td className="px-[2px] py-[2px]">{transaction.mode}</td>
