...
import { getAllSuspendedUsersReport } from "../../../service/subscriberReport";
import { IoMdArrowBack } from "react-icons/io";

export default function SuspendedCustomerReport() {
  ...
  // Fetch suspended users report
  useEffect(() => {
    const loadUsers = async () => {
      setLoading(true);
      try {
        const res = await getAllSuspendedUsersReport(page, limit, searchTerm);
        setUsers(res.data.data || []);
        setTotalPages(Math.ceil(res.data.total / limit));
      } catch (err) {
        console.error("Error fetching suspended users report:", err);
        setError("Failed to load suspended users report");
        toast.error("Failed to load suspended users report ❌");
      } finally {
        setLoading(false);
      }
    };
  ...
  return (
    <div className="p-6">
      <div className="flex items-center justify-between mb-4">
        <h1 className="text-xl font-semibold">Suspended Customer Report</h1>
      </div>
 <button
      ...
      {users.length === 0 ? (
        <p className="text-gray-500 text-[14px]">No suspended users found.</p>
      ) : (
        <>
          {/* Desktop Table View */}
              ...
              <tbody className="divide-y divide-gray-200">
                {users.map((user, index) => (
                  <tr key={user.id} className="hover:bg-gray-50 relative">
                    ...
                    <td className="px-[2px] py-[2px] hover:cursor-pointer hover:underline" onClick={() => handleView(user.id)}>{user.username}</td>
