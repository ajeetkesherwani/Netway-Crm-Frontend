...
import { getAllInactiveUsersReport } from "../../../service/subscriberReport";
...
export default function InactiveCustomerReport() {
  ...
  // Fetch inactive users report
  useEffect(() => {
    const loadUsers = async () => {
      setLoading(true);
      try {
        const res = await getAllInactiveUsersReport(page, limit, searchTerm);
        setUsers(res.data.data || []);
        setTotalPages(Math.ceil(res.data.total / limit));
      } catch (err) {
        console.error("Error fetching inactive users report:", err);
        setError("Failed to load inactive users report");
        toast.error("Failed to load inactive users report ❌");
      } finally {
        setLoading(false);
      }
    };
  ...
  return (
    <div className="p-6">
      <div className="flex items-center justify-between mb-4">
        <h1 className="text-xl font-semibold">Inactive Customer Report</h1>
 ...
 <button
      ...
      {users.length === 0 ? (
        <p className="text-gray-500 text-[14px]">No inactive users found.</p>
      ) : (
        <>
          {/* Desktop Table View */}
