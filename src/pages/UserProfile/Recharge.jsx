...
const UserRechargePackage = () => {
  ...
  const filteredPackages =
    searchTerm.trim() && searchTerm !== selectedPackage?.packageName
      ? packages.filter((pkg) =>
              ...
              <div className="grid grid-cols-2 gap-6">
                ...
                <div className="space-y-2.5">
                  <div className="flex items-center gap-3 relative">
                    ...
                    <div className="flex-1 relative" ref={packageDropdownRef}>
                      <div className="relative">
                        <input
                          type="text"
                          className={`w-full border border-gray-300 rounded pl-3 pr-8 py-1.5 text-xs font-medium focus:border-blue-500 focus:outline-none bg-white ${
                            !isRenew ? "cursor-pointer" : "cursor-not-allowed bg-gray-100"
                          }`}
                          ...
                          onFocus={() => !isRenew && setShowDropdown(true)}
                          onClick={() => !isRenew && setShowDropdown(true)}
                          disabled={isRenew}
                        />
                        <div
                          className={`absolute inset-y-0 right-0 pr-2.5 flex items-center ${
                            isRenew ? "cursor-not-allowed opacity-50" : "cursor-pointer"
                        ...
                        <ul className="absolute z-50 w-full bg-white border border-gray-300 rounded-b mt-1 max-h-48 overflow-y-auto shadow-lg text-xs">
                          {filteredPackages.length > 0 ? (
                            filteredPackages.map((pkg) => {
                              const isSelected =
                                selectedPackage?._id === pkg._id || form.packageId === pkg._id;
                              return (
                                <li
                                  key={pkg._id}
