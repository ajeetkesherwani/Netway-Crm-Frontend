...
import { getLcoWiseDetail } from "../../../../service/lcoWiseDetails";
...
export default function LcoWiseDetail() {
  const { type, resellerId,lcoId } = useParams(); // route: /dashboard/lco/:resellerId/:type
  ...
  const load = async () => {
    ...
    setError("");
    try {
      const res = await getLcoWiseDetail(type || "details", lcoId, { // ye lco id hai jo paras se get hoke jaa rahi hai, to ye as a reseller get ho rahi hai api page par
        filter,
        month: selectedMonth,
        year: selectedYear,
        page: 1,
      ...
      // API formats: { data: { result: [...] } } or { data: [...] } or direct array
      ...
      // Expect items like { label: "1 October", count: 0 } or { label, count }
      const labels = list.map((r) => String(r.label ?? ""));
      const values = list.map((r) => Number(r.count ?? r.totalUsers ?? r.total ?? 0));
        ...
        datasets: [
          {
            ...
            backgroundColor: labels.map((_, i) =>
              ["#36A2EB", "#4BC0C0", "#FF6384", "#FFCE56", "#9966FF"][i % 5]
            ),
            borderRadius: 6,
            maxBarThickness: 40,
          },
      ...
      setError(err?.message || "Failed to load LCO detail");
    } finally {
      setLoading(false);
    }
  };
  ...
  const options = {
    ...
    plugins: {
      ...
      title: { display: true, text: `${type ? type.toUpperCase() : "LCO"} — ${filter.charAt(0).toUpperCase() + filter.slice(1)} Wise` },
    ...
    layout: { padding: { top: 8, bottom: 28 } },
  };

  return (
    <div className="p-6">
      <div className="flex items-center justify-between mb-4">
        <h1 className="text-xl font-semibold">LCO Wise — {type || "details"}</h1>
      ...
      <div className="flex items-center gap-3 mb-4">
        ...
        {/* no search / no table as requested */}
      </div>
