...
//             className="px-4 py-2 text-sm border border-gray-300 rounded-md bg-white focus:outline-none focus:ring-2 focus:ring-blue-500 min-w-[160px]"
//           >
...
export default function PriceBookList() {
  ...
  // Auto-apply search when searchTerm or selectedType changes (live filtering)
  useEffect(() => {
    handleSearch();
        ...
        <div className="flex flex-col sm:flex-row sm:items-center gap-4">
          ...
          <div className="flex items-center bg-white rounded-md shadow-sm border border-gray-300">
            <input
              ...
              onChange={(e) => setSearchTerm(e.target.value)}
              onKeyPress={(e) => e.key === "Enter" && handleSearch()}
              placeholder="Search by assigned name..."
              className="px-4 py-2 text-sm outline-none w-64"
            />
            <button
              onClick={handleSearch}
