...
// import { updatePriceBook, getPriceBookDetails, getPackageList, getRetailerList, getLcoList, getLcoListByReseller } from "../../service/pricebook";
...
//   // Fetch data for prefill and dropdowns
//   useEffect(() => {
...
//           getLcoList(),
//         ]);
//         const priceBook = priceBookRes.data || null;
//         console.log(priceBook,packageRes,"-----------------------------price book data")
//         if (priceBook) {
//           setFormData({
//             priceBookName: priceBook.priceBookName || "",
//             fromDate: priceBook.fromDate || "",
//             toDate: priceBook.toDate || "",
//             status: priceBook.status || "active",
//             description: priceBook.description || "",
//             priceBookFor: priceBook.priceBookFor || "Reseller",
...
//           });
//           setSelectedPackages(
//             (priceBook.package || []).reduce((acc, pkg) => {
//               acc[pkg?.packageId] = pkg;
//               return acc;
//             }, {})
//           );
//           // Prefill selected resellers and LCOs
//           if (priceBook.priceBookFor === "Reseller" && priceBook.assignedTo.length > 0) {
//             setSelectedResellers(priceBook.assignedTo);
//           } else if (priceBook.priceBookFor === "Lco" && priceBook.assignedTo.length > 0) {
//             // Assume first assignedTo is reseller ID; fetch LCOs under it
//             const firstResellerId = priceBook.assignedTo[0];
//             setSelectedResellerForLco(firstResellerId);
//             try {
//               const res = await getLcoListByReseller(firstResellerId);
//               setLcosForSelectedReseller(res.data || []);
//               // Set selected LCOs from fetched data (filter assignedTo that are LCOs)
//               const lcoIds = priceBook.assignedTo.slice(1); // Assuming first is reseller
//               setSelectedLcos(lcoIds);
//             } catch (err) {
//               console.error("Error pre-filling LCOs:", err);
//             }
//           }
//         }
//         setPackages(packageRes.data ? packageRes.data : []);
...
export default function PriceBookUpdate() {
  ...
  const [formData, setFormData] = useState({
    ...
    description: "",
    priceBookFor: "Reseller",
    package: {}, // object with packageId as key
    assignedTo: [],
  });
