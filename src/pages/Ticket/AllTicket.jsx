...
export default function AllTicket() {
  ...
  // ✅ Handle search input (client-side)
  const handleSearch = (e) => {
    const value = e.target.value;
    setSearchTerm(value);
      ...
      <div className="overflow-x-auto bg-white shadow rounded-lg border border-gray-100">
        ...
        {filteredTickets.length > 0 ? (
          filteredTickets.map((ticket, index) => (
            <div
              key={ticket._id}
              className="grid grid-cols-2 md:grid-cols-9 items-center text-sm text-gray-700 border-b hover:bg-gray-50 px-4 py-3 transition duration-200"
            >
              <div>{(page - 1) * limit + index + 1}</div>
              <button
                onClick={() => handleViewTicket(ticket._id)}
